    ...
    export default {
        ...
        methods: {
            complie: async function() {
                ...
                let result = this.compiler({sources:{'tron':this.source},settings:{optimizer: {enabled: true, runs:0}}},1);
                console.log(result);
                this.loading = false;
                if (result.errors) {
                    this.success = false;
                        ...
                        do {
                            ...
                            if (transactionInfo.id) {
                                if (transactionInfo.receipt.result == "SUCCESS") {
                                    this.success = true;
                                    this.consoleResult += (`SUCCESSFULLY deployed ${this.currentContractDeployName}. Cost: ${(transactionInfo.receipt.energy_fee?FormatNumber(transactionInfo.receipt.energy_fee/ 1000000):0)} TRX, ${transactionInfo.receipt.energy_usage?FormatNumber(transactionInfo.receipt.energy_usage):0} energy`) + '\n';
                                    ...
                                    this.success = false;
                                    this.consoleResult += (`FAILED deploying ${this.currentContractDeployName}. You lost: ${(transactionInfo.receipt.energy_fee?FormatNumber(transactionInfo.receipt.energy_fee/ 1000000):0)} TRX`) + '\n';
